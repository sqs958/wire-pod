...
func deepseekRequest(transcribedText string) string {
	var userContent string

	if vars.APIConfig.STT.Language == "zh-CN" || vars.APIConfig.STT.Language == "zh" {
		userContent = "你是一个有用的机器人。请用简洁的中文回答用户的问题，不要太长。请直接回答问题，不要说\"好的\"或\"当然\"之类的开场白。\n\n用户的问题是：" + transcribedText
	} else {
		userContent = "You are a helpful robot called " + vars.APIConfig.Knowledge.RobotName + ". You will be given a question asked by a user and you must provide the best answer you can. It may not be punctuated or spelled correctly as the STT model is small. The answer will be put through TTS, so it should be a speakable string. Keep the answer concise yet informative. Here is the question: " + transcribedText
	}

	logger.Println("Making request to DeepSeek API...")
	...
	if err != nil {
		logger.Println(err)
		return "There was an error making the request to DeepSeek API."
	}
	defer resp.Body.Close()
	body, _ := io.ReadAll(resp.Body)
	...
	if err != nil || len(dsResp.Choices) == 0 {
		logger.Println("DeepSeek returned no response.")
		return "DeepSeek returned no response."
	}
	apiResponse := strings.TrimSpace(dsResp.Choices[0].Message.Content)
	logger.Println("DeepSeek response: " + apiResponse)
	return apiResponse
}
...
func deepseekKG(speechReq sr.SpeechRequest) string {
	transcribedText, err := sttHandler(speechReq)
	if err != nil {
		return "There was an error."
	}
	return deepseekRequest(transcribedText)
}
...
func KgRequest(speechReq sr.SpeechRequest) string {
	if vars.APIConfig.Knowledge.Enable {
		...
		} else if vars.APIConfig.Knowledge.Provider == "deepseek" {
			return deepseekKG(speechReq)
		}
	}
