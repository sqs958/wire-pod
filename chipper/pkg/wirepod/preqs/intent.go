...
func (s *Server) ProcessIntent(req *vtt.IntentRequest) (*vtt.IntentResponse, error) {
	...
	if !successMatched {
		if vars.APIConfig.Knowledge.IntentGraph {
			...
			var resp string
			if vars.APIConfig.Knowledge.Provider == "deepseek" {
				resp = deepseekRequest(transcribedText)
				logger.LogUI("DeepSeek response for device " + req.Device + ": " + resp)
				...
				resp = togetherRequest(transcribedText)
				logger.LogUI("Together response for device " + req.Device + ": " + resp)
			}
